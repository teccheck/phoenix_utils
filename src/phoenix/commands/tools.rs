...
pub fn backlight_test_mode(
    ...
    let rsp = validate_command_response_type(&rsp, CommandType::ToolsBacklightTestMode)?;
...
pub fn backlight_normal_mode(port: &mut Box<dyn SerialPort>) -> Result<(), Box<dyn Error>> {
    ...
    let rsp = validate_command_response_type(&rsp, CommandType::ToolsBacklightNormalMode)?;
...
pub fn led_test_mode(port: &mut Box<dyn SerialPort>, mode: u8) -> Result<(), Box<dyn Error>> {
    ...
    let rsp = validate_command_response_type(&rsp, CommandType::ToolsLedTestMode)?;
...
pub fn led_normal_mode(port: &mut Box<dyn SerialPort>) -> Result<(), Box<dyn Error>> {
    ...
    let rsp = validate_command_response_type(&rsp, CommandType::ToolsLedNormalMode)?;
